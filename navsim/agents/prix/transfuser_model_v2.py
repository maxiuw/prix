...
import copy
from navsim.agents.prix.transfuser_config import TransfuserConfig
# from navsim.agents.prix.transfuser_backbone import TransfuserBackbone
# from navsim.agents.prix.lss_backbone import LSSBackbone
from navsim.agents.prix.backbone_shared import TransfuserBackboneShared

from navsim.agents.prix.transfuser_features import BoundingBox2DIndex
from navsim.common.enums import StateSE2Index
from diffusers.schedulers import DDIMScheduler
from navsim.agents.prix.modules.conditional_unet1d import ConditionalUnet1D,SinusoidalPosEmb
import torch.nn.functional as F
from navsim.agents.prix.modules.blocks import linear_relu_ln,bias_init_with_prob, gen_sineembed_for_position, GridSampleCrossBEVAttention
from navsim.agents.prix.modules.multimodal_loss import LossComputer
...
class V2TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._config = config
        self._backbone =TransfuserBackboneShared() # TransfuserBackbone() #LSSBackbone() #TransfuserBackboneShared() #T
        H, W = 8, 32                     
        ...
        self._bev_downscale = nn.Conv2d(512, config.tf_d_model, kernel_size=1) # 512 for resnet34, 2048 for renset50
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        # print(f"1. camera_feature shape: {camera_feature}")
        ...
        # self.viz(cross_bev_feature_projected,camera_feature)
    ...
    def viz(self,features, image):
        import matplotlib.pylab as plt 
        activation_values = torch.mean(features, dim=2).squeeze(0)
        # 2. Reshape the sequence of 4096 values back into a 2D patch grid (32x128)
        # We know the patch grid dimensions from how the sequence was created.
        patch_grid_h, patch_grid_w = 32, 128
        activation_map = activation_values.reshape(patch_grid_h, patch_grid_w)


        # 3. Upsample the 32x128 map to the original image size and plot
        # Move data to CPU for plotting
        activation_map_cpu = activation_map.detach().cpu().numpy()
        image_to_plot = image.squeeze(0).permute(1, 2, 0).cpu().numpy()

        # Upsample using interpolate
        upsampled_map = F.interpolate(
            torch.from_numpy(activation_map_cpu).unsqueeze(0).unsqueeze(0),
            size=(256, 1024), # Original image H, W
            mode='bilinear',
            align_corners=False
        ).squeeze().numpy()
        ...
        ax.set_title("ViT-style Feature Activation Heatmap")
        ax.axis('off')
        fig.savefig('bevgrid.png')
